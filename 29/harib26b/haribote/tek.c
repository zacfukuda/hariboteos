...
static int tek_decode1(int siz, UCHAR *p, UCHAR *q) {
	...
	if ((dsiz = tek_getnum_s7s(&p)) > 0) {
		...
		bsiz = 1 << (((hed >> 1) & 0x0f) + 8);
		if (dsiz > bsiz || (hed & 0x21) != 0x01) return 1;
		if (hed & 0x40) tek_getnum_s7s(&p);
		if (tek_getnum_s7s(&p) != 0) return 1;
...
static int tek_lzrestore_stk2(int srcsiz, UCHAR *src, int outsiz, UCHAR *q) {
	...
	bylz = cbylz = 0;
	if (outsiz) {
		if (tek_getnum_s7s(&s7ptr)) return 1;
		do {
			j = 0;
			do {
				j++;
