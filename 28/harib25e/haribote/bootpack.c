...
void HariMain(void) {
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	if (finfo != 0) {
		...
		for (i = 0; i < 16 * 256; i++) {
			nihongo[i] = hankaku[i]; // Copy hankaku characters
		}
		for (i = 16 * 256; i < 16 * 256 + 32 * 94 * 47; i++) {
			nihongo[i] = 0xff; // Fullfill font with 0xff
		}
	}
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) { // Keyboard
				if (i < 0x80 + 256) { // Convert to the Symbol
				...
				if (s[0] != 0 && key_win != 0) { // Normal letter
				...
				if (i == 256 + 0x3c && key_shift != 0) { // Shift + F2
					if (key_win != 0) { // Focus on the new console
						keywin_off(key_win);
					}
					keywin_off(key_win);
					key_win = open_console(shtctl, memtotal);
				...
				if (mouse_decode(&mdec, i - 512) != 0) {
					...
					if ((mdec.btn & 0x01) != 0) { // Left button is clicked
						if (mmx < 0) {
							// Search sheet from the top
					...
					} else { // Left button is not clicked
						mmx = -1;
			...
			} else if (2024 <= i && i <= 2279) { // Close only console
